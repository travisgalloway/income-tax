...
splits). Read this before adding a column, a filter, or a sort key to `LawExplorer.tsx`, because several
...
has no `public_law` at all, fails the build with the law named. It is never dropped, because
dropping it removes a row from the table and shrinks the `N laws · $X.XXT scored` totals line with
no signal, publishing a wrong number. Before #33, §8 dropped silently and §9 threw, and the throw
is what both do now. The throw fires during `astro build`, because Astro server-renders this island even at
`client:load`. `join.ts` imports **types only**, because importing `src/data/index.ts` there would pull
...
## `d` and `d_caucus`: the basis a chart MUST state
...
one is active in visible prose, and not only in the control. D2 requires this stated in the UI, not
just toggleable.

## `house: null` and `senate: null`: absence, never unanimity

A null chamber means **no roll call exists**, almost always a voice vote, and never a unanimous or
...
margin is its Senate margin (96), and the function is total over all 23 laws
...
- Are excluded from every displayed cost total (`totalsOf` in `derive.ts` sums only non-null
  `score_t`).
- Sort **last in both directions** on the "Ten-year score" column, using a `nullsLast` comparator
...
check into a tolerance, because a ±0.02 tolerance is what let the two values disagree unnoticed.
...
- `senate` is a required object, and **`house` is `["object", "null"]`**, because a null chamber means no roll
