...
`HouseholdSpread.tsx`). Read this before charting either, because both carry a trap that looks like a
...
year, and never `0` (`test_absent_observations_are_null_not_zero`):
...
`_meta.gini_basis === 'families'`. The household Gini series runs about 0.47 to 0.49, noticeably
higher than the family series' 0.456 (2024). **A consumer must read the label from
`_meta.gini_basis` and never hardcode the word "families".** That is what
...
in **FY2025 dollars**, on a different deflator (CPI-U-RS against the GDP price index) and a
different base year. `UnitToggle` / `UNIT_LABEL` / `UNIT_PREFIX` (`charts/format.ts`) are built around the
...
years before it are not flat, and they are unobserved by this measure. `gini` runs back to 1947. A
...
`incomeGroups` stays typed as `Dataset<Record<string, unknown>>` in `src/data/index.ts`, and widening
...
That is the entire array, at **exactly two rows**, both nation-level top 1% share of income
...
nor 2022. That must render an explicit "no published observation in this range" state, and never an
...
- `data` is an array of at least 100 rows, and every row carries `y`, `top`, `mhi` and `gini`. The
...
- `_meta.title` names **one span per series**, derived from those per-series coverage blocks
  ...
  chart window. `MedianIncome` takes its domain from `seriesSpan(rows, 'mhi')`, and never from a
