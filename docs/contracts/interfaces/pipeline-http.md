...
is how they drifted apart (issue #40). One private core, `_retrieve`, now implements the
discipline exactly once, and the four public entry points choose a mode and adapt the result.
...
attributes and must stay that way, so neither may move inside a helper or a config object.
...
and preserved, because no caller needs it and adding it is an unrequested signature change.
...
1. **Any transport error, timeout or non-200 raises `SourceUnavailable`.** There is no code path
   ...
   and they carry the request `url`. For `post_json` that is the URL rather than its composite
   cache key.
...
4. **A failed request is never attempted a second time.** There is no retry and no backoff anywhere
   ...
   one loud failure rather than as a run that takes six times as long and fails anyway. Issue #40's body
...
Both shapes are depended on. 67 warm entries exist under `pipeline/.cache` at the time of
...
file. **A cache entry lacking the key the caller wants is a cache miss**, so the request goes to
the network and overwrites the entry in the requested shape.
...
fire where the old code crashed, so it removes no working path and converts a crash into a
...
already written it. The difference is not an inconsistency to reconcile, because it follows from
which function owns the write:
...
network outage, because an outage raises the same `SourceUnavailable` they assert, so a green run
of those two is weaker evidence than it looks.
