...
| `offsetting receipts` | Occurs on `/government` only inside a `<Figure note>`, which is a `string` prop and cannot carry a marker (`Figure.astro:37`), and inside prose that defines it in the same clause ("net of offsetting receipts, the basis used throughout this section"). |
