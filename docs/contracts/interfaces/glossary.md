...
**not a pipeline output**, because nothing emits it, it is hand-authored Markdown, and it has no
...
`src/data/index.ts`'s `assertDataset` is likewise not a third layer here, because it guards
...
2. Plain text makes "zero external hyperlinks on this page" structurally true rather than
   grep-true, because a Markdown body could grow an `[x](https://…)` at any time.
...
## `src/data/sections.ts`: one map of route to section anchors
...
a section was renamed, and it would drift silently, which is the failure `first_used` exists to catch.
...
`z.enum`, which is correct, because no glossary term's first prose use is on any of them.
...
## In-prose markers: the `<Term>` consumer boundary (#47)
...
| **`short` carries two jobs** | It is the `/glossary` entry's opening sentence *and* the popover's whole body, referenced by the trigger's `aria-describedby`, so it is in the accessible description of a link in running prose, with scripting on or off. |
...
> markable prose**, meaning text inside an element carrying `prose`, `standfirst` or `finding`, where
...
- **`<Figure>` props.** `ariaLabel`, `title`, `source`, `note` and `vintage` are `string` props
  validated at `Figure.astro:37-41`. They cannot carry markup, so captions are impossible rather
  than merely out of scope. That is why `.figure-caveat` is outside "markable prose" by
...
that a finding is terse and number-dense. Criterion 4 counts a finding as a first use, because a
reader meets `intragovernmental` in `/government` §2's finding a full paragraph before §2's prose,
so findings are now markable, and four markers sit in one. The constraint #47 was protecting still
...
a rename, or a new term left unmarked, fails there. Six terms are exceptions, listed explicitly in
...
See "In-prose markers" above for the boundary it actually uses. The marker is neither `<dfn>` nor
`<abbr>`, because the trigger has to be a real `<a href>` so it survives scripting off, and `<dfn>`
marks the *defining* instance of a term, which the `/glossary` entry is and a prose mention is not.
#47 wraps words and rewords no sentence, and #45 makes no prose edits at all.

**#49, an index of every term, every section and every figure.** Attaches to
...
(`astro check` and `npm run build`) and `check_glossary_sources` in `pipeline/lib/validate.py`
(`build.py --dry-run` and `pytest`). `check_sources` rule C now spans outputs **and** glossary
...
`pipeline/tests/test_prose.py`. One asserts that no marked term's surface form appears earlier in
markable prose than its marker, and the other that every content route marks every glossary term it
uses there. Neither carries a baseline, because seven measured violations sat on `prose.md` rule
3's fix-all practice.
**What is still not here**: which *sense* of a word is on the page. `real money` and `real terms`
are the same eight characters to a matcher, so the fix menu has two items, moving the marker or
rewording the earlier sentence. That is why Checklist item 4 in `docs/contracts/prose.md` stays NOT
EXECUTED.
