...
(`StatutoryVsEffective.tsx`). Read this before changing either builder or either island, because it exists
...
only, capital gains, AMT, NIIT, the 1981 part-year blend), and `curated/top_rates.yaml`'s `_comment`
...
parsed at 16 brackets: a 0% zero bracket `$0-$2,390`, then 11/12/14/15/16/18/20/23/26/30/34/38/42/
...
The output-side check is a `validate.py` invariant rather than a JSON-Schema one, because JSON
Schema cannot express "strictly increasing across array items". A duplicate floor in any year fails the build
...
government shutdown, and BLS stated it could not retroactively gather it, the first gap in this
...
consumes. Coverage is **1913-2018, 106 rows, no gaps**, and it does not reach 2019, where the fetched
CSV ends.
...
It is curated and frozen, because Table 23 ends at 2018 and was last saved 2021-01-13, and `xlrd` is
...
gaps, which is the half that stops a footnote-prefixed cell (`[19] 91.0` at 1954, `[24, 25] 70.0` at 1974,
...
A curated snapshot rather than a builder in its own right, in the same shape family as `debt_holders` and
...
narrowing accessor through `unknown`, because `astro check` rejected a single assertion for
...
The effective-rate companion, `cbo_effective_rates.json`, is a curated snapshot, and its schema is
...
- `s` requires all four filing statuses (`single`, `mfj`, `mfs`, `hoh`), each `["array", "null"]`.
  **`null` means the status did not exist that year**, covering 1913-1948 for `mfj` and `mfs` and
  1913-1951 for `hoh`, and it is never an empty array.
- Each bracket requires `r`, `lo`, `hi`, `rlo`, `rhi`. `lo` and `rlo` are plain numbers
  ...
  `["number", "null"]` with `not: {"const": 0}`**, because the top bracket's open ceiling is
  `null` and a ceiling written as `0` fails the build. That is the same rule
